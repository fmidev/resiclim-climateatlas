...
maximum 10-m wind speed, centred on a 31 d window, for the base period 1981–2010.
This threshold is used in the calculation of high wind speed events.
...
def save_as_dataarray(array, doy_values, da_u10max):
    
    da = xr.DataArray(data=array, dims=["doy","latitude","longitude",],
                                           coords={'doy':doy_values,
                                                   'longitude': da_u10max.longitude, 
                                                   'latitude':da_u10max.latitude})
    return da


# define the output data path to which the resulting percentiles are saved
outpath = '/projappl/project_2005030/climateatlas/ws_threshold.nc'
...
da_u10max = io_utils.read_daily_data_from_allas(fs, years, 'summer', '10m_u_component_of_wind','DMAX')
da_v10max = io_utils.read_daily_data_from_allas(fs, years, 'summer', '10m_v_component_of_wind','DMAX')
...
p90_array = np.empty((366, np.shape(da_u10max)[1], np.shape(da_u10max)[2]))


doy_values = np.unique(da_u10max['time.dayofyear'].values)
    
for day in doy_values:
    start2 = time.time()
    print('Calculating thresholds for doy '+str(day))

    dayofyear = da_u10max['time.dayofyear'] == day
    ...
    p90_array[int(day)-1,:, :] = np.nanpercentile(ws.sel(time=selection), 90, axis=0)
    
    end2 = time.time()
    print('Calculation lasted '+str(np.round(end2 - start2, 2))+' seconds')


p90_da = save_as_dataarray(p90_array, doy_values, da_u10max)
...
ds_out.attrs['title'] = 'Wind speed threshold'
